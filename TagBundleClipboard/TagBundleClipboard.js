...
(function () {
  ...
  // so only an empty box, or text that is not a number at all, means "unset". Testing
  // `n > 0` conflated the two and answered 5 to a deliberate 0, which is neither what
  // the box says nor what the description promises.
  function maxBundles(s) {
    var n = parseInt((s && s.a1MaxBundles) || '', 10);
    if (isNaN(n)) return DEFAULT_BUNDLES;
  ...
  // it wrong is silent in both directions - the wrong control is pasted into, or the right
  // one is read stale after a hand edit. See `pickControl`.
  function findControl(type, id, expectedIds) {
    return pickControl(_captures, function (c) {
  ...
  PasteRun.prototype.build = function () {
    ...
    checkInstalledVersion(function (installed) {
      ...
      staleReloadButton(self.staleEl);
  ...
  PasteRun.prototype.tagRow = function (r) {
    ...
    // Through `tagTip`, which draws it as a `title` and reopens it as a box with the tag's
    // picture above it - which for a list of names being ticked one by one is often the
    // fastest way to tell two of them apart.
    tagTip(row, r.tag.id, tagTitle(r.tag) + (r.protect ? '\n\n' + r.protect : ''));
