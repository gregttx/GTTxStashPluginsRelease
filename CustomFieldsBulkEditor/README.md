...
of a change, or a field set to an empty value. Hover it to see which. It is dropped from anything
you copy, so a copied line carries the empty value the entity really has; names of your own
containing `␀` are ordinary text and keep it.

**A value that is not text says so after it** — `num`, `bool` or `null`. Text is the ordinary case
and gets no mark; hovering any value names its type either way. `null` is the one worth having: a
...
- **Fields** is whether the entity carries any custom fields at all — **Any** *(default)*,
  **With fields**, or **Without fields**. It sits beside **Type** because it is a property of the
  entity rather than of a line. **Without fields** is how you reach the entities an **Add** is
  for.
...
- The filters are what decide the scope, so they are **unavailable while a write is running**. The
  counter directly above them says what is going on.
...
**Apply writes to exactly the entities the list is showing.** There is no scope selector: clear the
filters to cover everything the dialog read, and narrow them to cover less. The number of entities
that is appears under the list and on the button.

**A value that reads as a number is stored as one** — `5`, `-2`, `3.5` — which is what Stash's own
custom field editor does with the same text, so a field written from either place holds the same
thing and Stash's numeric filters compare it the same way. Anything else is stored as text. The
mark beside the box says which you are about to write. An entity already holding the same digits as
*text* is not treated as already correct — **Overwrite** puts the number there, which is how you fix
a field that was written the other way.
...
**Undo** appears and **Apply** goes, with **Rescan** between them; the way out is saying **Close** by then.
...
under a dialog you left open. It **replaces** the listing rather than adding a second one below it —
there is only ever one list of what the entities carry, and it is the current one. Everything else
in the log stays: every message, and the report of every Apply with its `before ⇒ after` lines.
...
- **The pane reads Name, then Description.** The field's name is in an editable box on the first
  ...
  the field is under that. That list marks a value that is not text — `num`, `bool` or `null` —
  the same way the bulk dialog does, since a null and an empty value look alike anywhere, and
  each entity in it hovers to a card of its own.
