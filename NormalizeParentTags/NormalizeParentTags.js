...
(function () {
  ...
  // Where this plugin's buttons sit when another's share the row. 15, in the gap between
  // `PropagateTagsAndPerformers` (10) and `MergePerformerTagsToScenes` (20): these two
  // rewrite the *hierarchy* on the scene in front of you rather than pulling metadata in
  // from elsewhere, which is the more consequential pair, so they sit nearer Delete than
  // the propagate buttons and further than the merge one that was there first. No
  // existing number moved, which is what the gaps are for.
  coop().order[PLUGIN_ID] = 15;
  ...
  function tagSummaryParts(graph, counts, verb, detail) {
    ...
    ids.forEach(function (tid, i) {
      ...
      parts.push({
        ...
        // A link and a tooltip on one segment: the hover says what the tag is, the
  ...
  // One log line for one tag on one entity, in either direction, as segments rather
  // than a string: everything the line names is a thing with a page, and a plan is
  // read to decide whether a change is right - "which of the two tags called Blonde
  // is that" and "what else is on this scene" are the questions it is read for.
  //
  // The tags are linked as well as the entity, and that is not the sibling's
  // arrangement: `PropagateTagsAndPerformers` hovers a tag to a card instead. This
  // plugin has no cards, and its whole subject is tags - one dead name in the middle
  ...
  function changeParts(graph, type, id, label, tid, reason, prefix) {
    ...
    // change is right, and "what else is on this scene" is half of that. A Scene Marker
    // has neither - no page, and no by-id query in Stash to ask with.
    parts.push({ text: type.label + ' ' + label, href: entityHref(type.key, id),
      ent: { type: type.key, id: id } });
    parts.push({ text: ' - ' });
    // The tags on the line hover to the same card as the entity does - the recap's own
    // tooltip says what a tag is once per run, and this line is where the decision is
    // actually made. It is the shared card rather than that tooltip because the card
    // carries the tag's picture, and because it is one fetch this plugin does not have
  ...
  // Every caller logs the same two things about a line, so they say it once.
  function logChange(run, kind, parts) {
    run.log(kind, partsText(parts), parts);
  }
  ...
  // One entity, planned exactly as the library walk plans each of its own - same
  // `planEntity`, same ctx, same plan entry, same log lines. A second planner here is how
  // a button and the task come to disagree about what a scene needs.
  function scanOne(type, id, mode, ctx) {
    return gqlRequest(autoEntityQuery(type), { ids: [String(id)] }).then(function (data) {
      var list = (data[type.find] || {})[type.node] || [];
      ctx.run.total[type.key] = list.length;
      ctx.run.scanned[type.key] = list.length;
      if (!list.length) {
        ctx.run.log('WARN', type.label + ' ' + id + ' could not be read; it may have gone ' +
      ...
      ctx.run.log('ERROR', type.label + ' ' + id + ' could not be read: ' + e.message);
      ctx.run.errors++;
    });
  }
    ...
    return gqlRequest(bulkMutation(batch.type), {
      ...
      batch.entries.forEach(function (entry) {
        batch.tagIds.forEach(function (tid) {
          ...
          logChange(run, mode,
            changeParts(graph, batch.type, entry.id, entry.label, tid, null, 'Undo - '));
  ...
  // `scope` is `{ type, id, label, mode }` or null. A scoped run is the same planner over
  // one named entity: the manual buttons open it, so what a click shows is by construction
  // what the library-wide task would show for that scene. It lives on the run rather than
  // in the call that started it because **Rescan re-enters `begin()`**, and a rescan of a
  ...
  Run.prototype.build = function () {
    ...
    head.appendChild(el('div', 'npt-title', PLUGIN_SHORT_NAME + ' - ' +
      stripEllipsis(this.taskName) + (this.scope ? ' - ' + this.scope.label : '')));
  ...
  Run.prototype.begin = function () {
    ...
    loadSettings().then(function (loaded) {
      ...
      // A scoped run covers one entity in one direction, and the selectors are hidden for
      // it - they steer a pass this one cannot reach.
      ...
      if (self.scope) {
        self.log('INFO', 'Scoped to ' + self.scope.label + ', ' +
          MODE_TOKEN[self.scope.mode] + ' only. Every exclusion filter still applies.');
      } else {
      ...
      return gqlRequest(tagQuery(self.settings), null).then(function (data) {
        ...
        types.forEach(function (t) {
          chain = chain.then(function () {
            ...
            self.renderProgress();
            return self.scope
              ? scanOne(t, self.scope.id, self.scope.mode, ctx)
              : scanType(t, self.modes[t.key], ctx);
          });
        });
  ...
  // hand - which is why the captions carry no "...": the dots are this repo's mark for a
  // click that opens a dialog first, and a staging button *is* its own plan. Close the
  // form without saving and nothing happened.
  //
  // Where staging is not available - a Stash too old to expose `PluginApi` component
  ...
  //
  // Stash's `TagSelect`, reached through `PluginApi`'s component patching. A
  // before-patch returns the argument list for the real render, so returning props
  // untouched makes this a pure observer.
  var _tagCaptures = [];
  var TAG_CAPTURE_LIMIT = 10;
  ...
  function installTagSelectPatch() {
    ...
    try {
      api.patch.before('TagSelect', function (props) {
        if (props && props.isMulti && typeof props.onSelect === 'function') {
      ...
      console.warn('[npt] could not patch TagSelect:', e);
      return false;
    }
  }
  ...
  // Display fields for the chips Stash draws in the box. The hierarchy query does not
  // carry them - it reads every tag in the library and does not need them - so they are
  ...
  function tagChips(ids) {
    ...
    return gqlRequest('query NPTChips($ids: [ID!]) { findTags(tag_filter: { id: ' +
      ...
      // still selects the right tag; a tag silently dropped because its decoration could
      // not be read is a staged list quietly missing something the button promised. So
      // the query *enriches* a list built from the hierarchy rather than being it.
      .then(function (found) {
        var byId = {};
  ...
  function stageScene(check, mode) {
    ...
    if (mode === MODE_PRUNE) {
      var drop = {};
      delta.remove.forEach(function (id) { drop[String(id)] = true; });
      var kept = (control.values || []).filter(function (t) { return !drop[String(t.id)]; });
      var removed = (control.values || []).length - kept.length;
      if (!removed) return Promise.resolve({ status: 'nochange' });
      control.props.onSelect(kept);
    ...
    var have = {};
    (control.values || []).forEach(function (t) { have[String(t.id)] = true; });
    var wanted = delta.add.map(String).filter(function (id) { return !have[id]; });
    if (!wanted.length) return Promise.resolve({ status: 'nochange' });
    return tagChips(wanted).then(function (chips) {
  ...
  // The caption is resolved per tick, not at build time: whether the click opens a dialog
  // depends on whether this Stash can be patched, and a button promising a dialog it no
  // longer opens is worse than no promise at all. `_nptLabel` holds the intended caption,
  // because a click overwrites `textContent` while a flash is in flight.
  var FLASH_MS = 1400;
  var _flashToken = 0;
  ...
  function onMutation() {
    ...
    _busPending = setTimeout(function () { _busPending = null; npTick(); }, 100);
  }
  ...
  if (window.addEventListener) {
    ...
    window.addEventListener('popstate', function () { setTimeout(npTick, 300); });
  }
  ...
  // The tag control is captured through a component patch, and `PluginApi` may not be on
  // the page yet at script bottom. Tried again on load, like the sibling's.
  if (!installTagSelectPatch() && window.addEventListener) {
